...
const getTextColor=(path: string)=>{
  for (let index = 0; index < routes.length; index++) {
    if(routes[index].href==path){
      return routes[index].text_color
    }
  }
}
const style1 = ' border-b-2 border-transparent transition duration-700 hover:border-inherit hover:transition hover:duration-700'
      ...
      <div className='flex justify-between container mx-auto mt-8 items-center'>
        ...
        <div className={'flex text-base space-x-10 tracking-wider'}>
            {routes.map((route,index)=>(
              <div key={route.id} className={(path==route.href&&'border-inherit')+ style1+textColor}>
                <Link href={route.href} className={roboto_mono.className}>{route.title}</Link>
              </div>
            ))}
        </div>
        <BsPerson className={'cursor-pointer size-8 '+style1+textColor} />
      </div>
    </div>
  )
}
