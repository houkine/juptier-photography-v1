                        ...
                        <div className='flex justify-between mt-10 w-full space-x-5'>
                            {set.sub_gallery.map((photo, index) => (
                                <div className='flex w-1/3'>
                                    <img src={photo.src} alt={photo.alt} className='flex m-auto rounded-md cursor-pointer shadow-lg hover:shadow-2xl transition duration-200' onClick={() => alert('we are working on it')} key={index} />
                                </div>
                            ))}
                        </div>
                    </li>
